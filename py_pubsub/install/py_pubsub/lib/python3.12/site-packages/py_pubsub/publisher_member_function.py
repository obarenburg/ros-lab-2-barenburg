...
class MinimalPublisher(Node):
    ...
    def timer_callback(self):
        if ((self.front_sensor != None) and (self.left_sensor != None)):
            ...
            if ((self.front_sensor > 3.5) and (self.left_sensor >= 2)):
                left_adjust = Twist(linear=Vector3(x=0.25, y=0.0, z=0.0), angular=Vector3(x=0.0, y=0.0, z=0.5))
                self.publisher_.publish(left_adjust)
            elif ((self.front_sensor < 4) or (self.left_sensor <= 2.5)):
                right_adjust = Twist(linear=Vector3(x=0.25, y=0.0, z=0.0), angular=Vector3(x=0.0, y=0.0, z=-0.5))
